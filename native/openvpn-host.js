#!/usr/bin/env node
/**
 * Native messaging host for VPN CX Net: start/stop OpenVPN from the browser extension.
...
function start(configPath) {
  if (fs.existsSync(PID_FILE)) {
    try {
      ...
      return {
        success: false,
        error: "OpenVPN is already running (PID " + pid + ")",
      };
    } catch (_) {
...
function stop() {
  if (!fs.existsSync(PID_FILE)) {
    return {
      success: false,
      error: "OpenVPN was not started by this extension",
    };
  }
  let pid;
  try {
